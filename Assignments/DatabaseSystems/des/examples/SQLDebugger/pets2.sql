...
create or replace view CatsOrDogsOwner(id,aname,specie) as
  ...
  where O.id = PO.id and P.code = PO.code and (specie='cat' or specie='dog');
...
create or replace view Guest(id,name) as
  select id, name 
  from Owner natural inner join NoCommonName natural inner join LessThan6;
