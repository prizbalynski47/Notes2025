...
:- dynamic(relCts/0). % This allows for knowing each node in the debug graph
relCts :-
  relation(q,2),
  relation(r,1),
  relation(t,2).
...
incompRelCts :-
  incompRel(t,'_nht').
bodyCts :-
  bodyRel('_t1',[(q,pos,'_wt11','_mt11'),(r,pos,'_wt12','_mt12')]),
  bodyRel('_t2',[(r,pos,'_wt21','_mt21')]).
connectedSubgraphCts :-
  connectedSubgraph([p],[q,r,s]),
  connectedSubgraph([q],[]),
  connectedSubgraph([r],[]),
  connectedSubgraph([s],[r,t]),
  connectedSubgraph([t],[q,r]).
cts :-
  relCts,
  subRelCts,
  incompRelCts,
